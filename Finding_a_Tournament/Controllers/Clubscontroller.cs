...
namespace Controllers
{
    ...
    public class Clubscontroller : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody]ClubsCreateRequest person)
        {
            var validationResult = await _createValidator.ValidateAsync(person);

            if(!validationResult.IsValid)
                return UnprocessableEntity(validationResult.Errors.Select(x => $"Error: {x.ErrorMessage}"));

            var entity = _mapper.Map<ClubsCreateRequest, Club>(person);
